<template>
  <layouts-header></layouts-header>
  <!-- Breadcrumb -->
  <div class="breadcrumb-bar text-center">
    <div class="container">
      <div class="row">
        <div class="col-md-12 col-12">
          <h2 class="breadcrumb-title mb-2">Become an Instructor</h2>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb justify-content-center mb-0">
              <li class="breadcrumb-item">
                <router-link to="/home/index">Home</router-link>
              </li>
              <li class="breadcrumb-item active" aria-current="page">
                Become an Instructor
              </li>
            </ol>
          </nav>
        </div>
      </div>
    </div>
  </div>
  <!-- /Breadcrumb -->

  <!-- share your knowledge -->
  <div class="share-your-knowledge">
    <div class="container">
      <div class="row">
        <div class="col-lg-7 pe-xl-5">
          <div class="share-knowledge-content">
            <div class="section-header">
              <span
                class="fw-medium text-secondary text-decoration-underline mb-2 d-inline-block"
                >Share Knowledge</span
              >
              <h2>Share Your Knowledge. Inspire the Future.</h2>
              <p>
                Share your knowledge, inspire learners worldwide, and earn while doing
                what you love. Join a community of experts transforming education through
                engaging and accessible content.
              </p>
            </div>
            <div class="row">
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-body">
                    <div class="d-flex align-items-center">
                      <div class="me-2">
                        <span
                          class="bg-secondary-transparent d-flex justify-content-center align-items-center p-3 rounded-pill"
                          ><img src="@/assets/img/icons/instructor-icon1.svg" alt="img"
                        /></span>
                      </div>
                      <div>
                        <h6 class="mb-1">Flexible Work</h6>
                        <p class="text-truncate line-clamb-1">Teach at your own pace.</p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-body">
                    <div class="d-flex align-items-center">
                      <div class="me-2">
                        <span
                          class="bg-secondary-transparent d-flex justify-content-center align-items-center p-3 rounded-pill"
                          ><img src="@/assets/img/icons/instructor-icon2.svg" alt="img"
                        /></span>
                      </div>
                      <div>
                        <h6 class="mb-1">Earning Potential</h6>
                        <p class="text-truncate line-clamb-1">Monetize your expertise.</p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-body">
                    <div class="d-flex align-items-center">
                      <div class="me-2">
                        <span
                          class="bg-secondary-transparent d-flex justify-content-center align-items-center p-3 rounded-pill"
                          ><img src="@/assets/img/icons/instructor-icon3.svg" alt="img"
                        /></span>
                      </div>
                      <div>
                        <h6 class="mb-1">Impact</h6>
                        <p class="text-truncate line-clamb-1">Reach and educate</p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-body">
                    <div class="d-flex align-items-center">
                      <div class="me-2">
                        <span
                          class="bg-secondary-transparent d-flex justify-content-center align-items-center p-3 rounded-pill"
                          ><img src="@/assets/img/icons/instructor-icon4.svg" alt="img"
                        /></span>
                      </div>
                      <div>
                        <h6 class="mb-1">Support</h6>
                        <p class="text-truncate line-clamb-1">
                          Access to dedicated support
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <a
              href="javascript:void(0);"
              class="btn register-btn-1 d-inline-flex align-items-center"
              >Register Now <i class="isax isax-arrow-right-3 ms-1"></i
            ></a>
          </div>
        </div>
        <div class="col-lg-5">
          <div class="share-your-knowledge-img d-none d-lg-flex">
            <img
              src="@/assets/img/shapes/shape-4.png"
              alt="img"
              class="img-fluid become-instructor-bg-02"
            />
            <img
              src="@/assets/img/shapes/shape-5.png"
              alt="img"
              class="img-fluid become-instructor-bg-01"
            />
            <img
              src="@/assets/img/shapes/shape-3.png"
              alt="img"
              class="img-fluid become-instructor-bg-03"
            />
            <img
              src="@/assets/img/feature/feature-5.jpg"
              alt="img"
              class="img-fluid rounded-4 become-instructor-bg-04"
            />
            <img
              src="@/assets/img/feature/feature-6.jpg"
              alt="img"
              class="img-fluid rounded-4 become-instructor-bg-05"
            />
            <img
              src="@/assets/img/shapes/shape-7.svg"
              alt="img"
              class="img-fluid become-instructor-bg-06"
            />
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- share your knowledge -->

  <!-- how it works -->
  <div class="how-it-works-sec bg-light-900">
    <div class="container">
      <div class="section-header">
        <span
          class="fw-medium text-secondary text-decoration-underline mb-2 d-inline-block"
          >Our Workflow</span
        >
        <h2>How It Works</h2>
        <p>Turn Your Expertise into Impact in Just 3 Simple Steps!</p>
      </div>
      <div class="row row-gap-4">
        <div class="col-lg-4">
          <div class="share-knowledge-item-2">
            <img src="@/assets/img/icons/how-it-works-1.svg" alt="img" />
            <h5 class="mt-3 mb-2">Apply & Get Approved</h5>
            <p class="text-truncate line-clamb-2">
              Submit your application and get approved to access the platform
            </p>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="share-knowledge-item-2">
            <img src="@/assets/img/icons/how-it-works-2.svg" alt="img" />
            <h5 class="mt-3 mb-2">Create & Upload Content</h5>
            <p class="text-truncate line-clamb-2">
              Develop and upload your courses, including videos, lessons, quizzes, and
              assignments.
            </p>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="share-knowledge-item-2">
            <img src="@/assets/img/icons/how-it-works-3.svg" alt="img" />
            <h5 class="mt-3 mb-2">Teach & Earn</h5>
            <p class="text-truncate line-clamb-2">
              Reach learners worldwide, teach, and start earning while making an impact.
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- how it works -->

  <!-- counter -->
  <section class="counter-sec">
    <div class="container">
      <div class="row gy-3">
        <div class="col-xl-3 col-md-6">
          <div class="card border-0 mb-0">
            <div class="card-body">
              <div class="d-flex align-items-center">
                <div class="counter-icon">
                  <img src="@/assets/img/icons/counter-icon1.svg" alt="img" />
                </div>
                <div class="count-content">
                  <h4 class="text-info">
                    <span class="count-digit"
                      ><vue3-autocounter
                        class="counter"
                        ref="counter"
                        :startAmount="0"
                        :delay="3"
                        :endAmount="10"
                        :duration="5"
                        :autoinit="true" /></span
                    >K
                  </h4>
                  <p>Online Courses</p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-md-6">
          <div class="card border-0 mb-0">
            <div class="card-body">
              <div class="d-flex align-items-center">
                <div class="counter-icon">
                  <img src="@/assets/img/icons/counter-icon2.svg" alt="img" />
                </div>
                <div class="count-content">
                  <h4 class="text-warning">
                    <span class="count-digit"
                      ><vue3-autocounter
                        class="counter"
                        ref="counter"
                        :startAmount="0"
                        :delay="3"
                        :endAmount="200"
                        :duration="5"
                        :autoinit="true" /></span
                    >+
                  </h4>
                  <p>Expert Tutors</p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-md-6">
          <div class="card border-0 mb-0">
            <div class="card-body">
              <div class="d-flex align-items-center">
                <div class="counter-icon">
                  <img src="@/assets/img/icons/counter-icon3.svg" alt="img" />
                </div>
                <div class="count-content">
                  <h4 class="text-skyblue">
                    <span class="count-digit"
                      ><vue3-autocounter
                        class="counter"
                        ref="counter"
                        :startAmount="0"
                        :delay="3"
                        :endAmount="6"
                        :duration="5"
                        :autoinit="true" /></span
                    >K+
                  </h4>
                  <p>Certified Courses</p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="col-xl-3 col-md-6">
          <div class="card border-0 mb-0">
            <div class="card-body d-flex align-items-center">
              <div class="counter-icon">
                <img src="@/assets/img/icons/counter-icon4.svg" alt="img" />
              </div>
              <div class="count-content">
                <h4 class="text-lightgreen">
                  <span class="count-digit"
                    ><vue3-autocounter
                      class="counter"
                      ref="counter"
                      :startAmount="0"
                      :delay="3"
                      :endAmount="60"
                      :duration="5"
                      :autoinit="true" /></span
                  >K+
                </h4>
                <p>Online Students</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- counter -->

  <!-- register -->
  <div class="register-sec">
    <div class="container">
      <div class="row align-items-center">
        <div class="col-lg-6">
          <div class="rounded-4 pe-lg-5">
            <img
              class="img-fluid rounded-5 d-none d-lg-flex"
              src="@/assets/img/feature/feature-4.jpg"
              alt="img"
            />
          </div>
        </div>
        <div class="col-lg-6">
          <div class="register-section p-4 p-sm-5 p-md-6">
            <h5 class="mb-2">Register</h5>
            <p>Your email address will not be published. Required fields are marked *</p>
            <form>
              <div class="row">
                <div class="col-lg-12">
                  <div class="mb-3">
                    <label class="form-label mb-1"
                      >Name<span class="ms-1 text-danger">*</span></label
                    >
                    <input type="text" class="form-control" />
                  </div>
                </div>
                <div class="col-lg-12">
                  <div class="mb-3">
                    <label class="form-label mb-1"
                      >Email<span class="ms-1 text-danger">*</span></label
                    >
                    <input type="email" class="form-control" />
                  </div>
                </div>
                <div class="col-lg-12">
                  <div class="mb-3">
                    <label class="form-label mb-1"
                      >Phone Number<span class="ms-1 text-danger">*</span></label
                    >
                    <input type="tel" class="form-control" />
                  </div>
                </div>
                <div class="col-lg-12">
                  <div class="mb-3 position-relative">
                    <label class="form-label mb-1"
                      >Password<span class="ms-1 text-danger">*</span></label
                    >
                    <div class="position-relative">
                      <input
                        :type="showPassword ? 'text' : 'password'"
                        class="pass-inputa form-control"
                      />
                      <span
                        @click="togglePassword"
                        class="isax toggle-passworda text-gray-7 fs-14"
                        :class="showPassword ? 'isax-eye' : 'isax-eye-slash'"
                      >
                      </span>
                    </div>
                  </div>
                </div>
                <div class="col-lg-12">
                  <div class="mb-4">
                    <label class="form-label mb-1"
                      >Confirm Password<span class="ms-1 text-danger">*</span></label
                    >
                    <div class="position-relative">
                      <input
                        :type="showPasswordOne ? 'text' : 'password'"
                        class="pass-inputa form-control"
                      />
                      <span
                        @click="togglePasswordOne"
                        class="isax toggle-passworda text-gray-7 fs-14"
                        :class="showPasswordOne ? 'isax-eye' : 'isax-eye-slash'"
                      >
                      </span>
                    </div>
                  </div>
                </div>
                <div class="col-lg-12">
                  <button
                    type="submit"
                    class="btn btn-secondary btn-lg w-100 justify-content-center"
                  >
                    Submit
                  </button>
                </div>
              </div>
            </form>
          </div>
        </div>
      </div>

      <div class="become-an-instructor rounded-2 bg-primary position-relative mt-5 p-5">
        <img
          src="@/assets/img/shapes/instructor-bg-1.png"
          alt="img"
          class="instructor-bg-1"
        />
        <img
          src="@/assets/img/shapes/instructor-bg-2.png"
          alt="img"
          class="instructor-bg-2"
        />
        <div class="row align-items-center">
          <div class="col-lg-8">
            <h3 class="text-white mb-2 mblg-3">Become an Instructor</h3>
            <p class="text-light">
              Turn your expertise into impactful courses and inspire learners worldwide.
              Join our community of instructors and start your journey today!
            </p>
          </div>
          <div class="col-lg-4 d-flex justify-content-lg-end justify-content-center">
            <a href="#" class="btn btn-secondary btn-lg mt-3 mt-lg-0"
              >Start Teaching Today</a
            >
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- register -->

  <!-- Footer -->
  <footer-two></footer-two>
  <!-- /Footer -->
</template>
<script>
export default {
  data() {
    return {
      showPassword: false,
      showPasswordOne: false,
    };
  },
  methods: {
    togglePassword() {
      this.showPassword = !this.showPassword;
    },
    togglePasswordOne() {
      this.showPasswordOne = !this.showPasswordOne;
    },
  },
};
</script>
